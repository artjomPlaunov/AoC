...
func main() {
  ...
  for _, room := range rooms {
    name := decrypt(room.name, room.id)
    if strings.Contains(name, "pole") {
      fmt.Println(name, room.id)
    }
  }
}
...
func isValidRoom(room Room) bool {
  ...
  letters := make([]byte, 0)
  for _,v := range room.name {
    if counts[byte(v)] == 0 {
      letters = append(letters, byte(v))
    }
    counts[byte(v)] += 1
  }
  ...
  return string(letters[:5]) == room.checksum
}

func parse_room(s string) Room {
  nameRegex, _ := regexp.Compile(`[a-z]+-`)
  var name strings.Builder
  for _, v := range nameRegex.FindAllString(s, -1) {
    name.WriteString(v[:len(v)-1])
  }

  idRegex, _ := regexp.Compile(`[\d]+`)
  var id int
  for _,v := range idRegex.FindAllString(s,-1) {
    id,_ = strconv.Atoi(v)
  }

  checksumRegex, _ := regexp.Compile(`\[[a-z]+\]`)
  var checksum string
  for _,v := range checksumRegex.FindAllString(s, -1) {
    checksum = v[1:len(v)-1]
  }

  return Room{name.String(),id,checksum}
}

var raw_input = `fubrjhqlf-edvnhw-dftxlvlwlrq-803[wjvzd]
